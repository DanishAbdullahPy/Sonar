...
export default function SpacecraftTracker() {
  ...
  const animateSatellite = () => {
    ...
    // Use real API data if available, otherwise use simulated data
    let satelliteX, satelliteY, lat, lon;

    if (satelliteData.positions && satelliteData.positions.length > 0) {
      // Use real API data for positioning
      const currentTime = Date.now();
      const positions = satelliteData.positions;

      // Find the closest position in time
      let closestIndex = 0;
      let minTimeDiff = Math.abs(new Date(positions[0].time).getTime() - currentTime);

      for (let i = 1; i < positions.length; i++) {
        const timeDiff = Math.abs(new Date(positions[i].time).getTime() - currentTime);
      ...
      const closestPos = positions[closestIndex];
      lat = parseFloat(closestPos.coordinates.latitude);
      lon = parseFloat(closestPos.coordinates.longitude);

      // Convert lat/lon to canvas coordinates
      satelliteX = centerX + (lon / 180) * orbitRadius;
      satelliteY = centerY - (lat / 90) * orbitRadius;
    } else {
      // Fallback to simulated positioning
      const angle = time * 0.5; // Adjust speed of orbit
      satelliteX = centerX + Math.cos(angle) * orbitRadius;
      satelliteY = centerY + Math.sin(angle) * orbitRadius * 0.9;

      // Calculate real-world coordinates (simplified)
      lat = parseFloat((90 - (satelliteY / canvas.height) * 180).toFixed(2));
      lon = parseFloat(((satelliteX / canvas.width) * 360 - 180).toFixed(2));
    }
    ...
    ctx.font = '12px Arial';
    ctx.fillText(`${lat.toFixed(2)}°, ${lon.toFixed(2)}°`, satelliteX + 15, satelliteY - 10);
  ...
  return (
    <div className="bg-[#151b2c]/80 backdrop-blur-md border border-gray-800 rounded-2xl shadow-xl p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          {mockSatellites.map((sat) => (
            ...
            >
              ...
              <div className="text-xs mt-1">{sat.description}</div>
            </button>
          ))}
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <h3 className="text-lg font-semibold text-white mb-2">
                  {satelliteData.name} ({satelliteData.id.toUpperCase()})
                </h3>
                <div className="text-sm text-gray-300">
                  <div className="mb-1">
                    <span className="text-gray-400">Current Position:</span>
                    <span className="ml-2">
                      {satelliteData.position.latitude}°, {satelliteData.position.longitude}°
                    </span>
                  </div>
                  <div className="mb-1">
                    <span className="text-gray-400">Altitude:</span>
                    <span className="ml-2">{satelliteData.position.altitude} km</span>
                  </div>
                  <div className="mb-1">
                    <span className="text-gray-400">Velocity:</span>
                    <span className="ml-2">{satelliteData.velocity} km/s</span>
                  </div>
                </div>
              </div>
              <div>
                <h3 className="text-lg font-semibold text-white mb-2">Next Pass</h3>
                <div className="text-sm text-gray-300">
                  <div className="mb-1">
                    <span className="text-gray-400">Start:</span>
                    <span className="ml-2">{formatDate(satelliteData.nextPass.startTime)}</span>
                  </div>
                  <div className="mb-1">
                    <span className="text-gray-400">End:</span>
                    <span className="ml-2">{formatDate(satelliteData.nextPass.endTime)}</span>
                  </div>
                  <div className="mb-1">
                    <span className="text-gray-400">Max Elevation:</span>
                    <span className="ml-2">{satelliteData.nextPass.maxElevation}°</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="mt-4 text-xs text-gray-400 flex items-center justify-between">
        <div>
          Data Source: {satelliteData && !satelliteData.isMock ? 
            'NASA SSCWeb API' : 
